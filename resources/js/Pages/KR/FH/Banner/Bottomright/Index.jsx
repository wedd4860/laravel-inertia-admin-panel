...
import { KR_SITE_NUMBER_GROUP, KR_FH_CHANNELING_SITE_GROUP } from "@/Stores/Constants";
...
const Index = ({ auth, banners, queryParams = null, success }) => {
    ...
    const aFHSiteNumber = KR_FH_CHANNELING_SITE_GROUP;
    ...
    const searchFieldChanged = (name, value) => {
        ...
        router.get(route("kr.fh.banner.bottomright.index"), queryParams);
    };
    ...
    const onSubmit = (e) => {
        ...
        destroy(route("kr.fh.banner.bottomright.destroy", data.idx), {
            preserveScroll: true,
            onSuccess: ({ props }) => {
                if (props.success != "") {
    ...
    return (
        <AuthenticatedLayout member={auth.member}>
            <Head title="배너 관리 : 출조낚시왕 메인 우측 하단 배너 - index" />

            <div className="px-6 py-4 overflow-hidden shadow-sm">
                <Link
                    href={route("kr.fh.banner.bottomright.create")}
                            ...
                            <tr className="text-nowrap">
                                <th className="px-3 py-2">
                                    ...
                                    >
                                        ...
                                        {aSiteNumberGroup.filter(group => aFHSiteNumber.includes(group.id)).map((group) => (
                                            <option
                                                key={group.id}
                                                value={group.id}
                                            >
