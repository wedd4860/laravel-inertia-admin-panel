...
const Index = ({ auth, events, queryParams = null }) => {
    ...
    const searchFieldChanged = (name, value) => {
        ...
        router.get(route("kr.lh.event.integration.management.index"), queryParams);
    };
    ...
    return (
        <AuthenticatedLayout member={auth.member}>
            <Head title="이벤트 관리 : 통합 이벤트 리스트 - index" />

            <div className="px-6 py-4 overflow-hidden shadow-sm">
                <Link
                    href={route("kr.lh.event.integration.management.create")}
                                ...
                                <tr key={item.event_index} className="border-b">
                                    ...
                                    <td className="px-3 py-2 text-center">
                                        ...
                                        {item.event_type == 1 ? (
                                            <Link
                                                href={route(
                                                    "kr.lh.event.integration.select.index",
                                                    item.event_index
                                                )}
                                                className="mr-1 float-left border-0 bg-slate-400 hover:bg-slate-300 py-1 px-2 text-gray-800 rounded transition-all text-white text-sm"
                                            >
                                                캐릭터 선택 로그
                                            </Link>
                                        ) : null }
                                        <Link
                                            href={route(
                                                "kr.lh.event.integration.receive.index",
                                                item.event_index
                                            )}
                                            className="float-right border-0 bg-slate-500 hover:bg-slate-300 py-1 px-2 text-gray-800 rounded transition-all text-white text-sm"
                                        >
                                            보상 수령 로그
                                        </Link>
                                    </td>
