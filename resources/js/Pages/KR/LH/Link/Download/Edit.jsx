...
const Edit = ({ auth, download, success }) => {
    const { data, setData, patch, errors, reset, processing } = useForm({
        idx: download.idx,
        imageTargetlink: download.imageTargetlink,
    });
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        if (!data.imageTargetlink) {
            return false;
        }
        patch(route("kr.lh.link.download.update"), {
            preserveScroll: true,
            onSuccess: ({ props }) => {
                if (props.success) {
    ...
    return (
        <AuthenticatedLayout member={auth.member}>
            <Head title="다운로드링크 관리 일반서버 - edit" />
