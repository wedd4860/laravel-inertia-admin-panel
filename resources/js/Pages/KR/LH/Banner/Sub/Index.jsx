...
const Index = ({ auth, banners, queryParams = null, success }) => {
    ...
    const searchFieldChanged = (name, value) => {
        ...
        router.get(route("kr.lh.banner.sub.index"), queryParams);
    };
    ...
    const onSubmit = (e) => {
        ...
        destroy(route("kr.lh.banner.sub.destroy", data.idx), {
            preserveScroll: true,
            onSuccess: ({ props }) => {
                if (props.success != "") {
    ...
    return (
        <AuthenticatedLayout member={auth.member}>
            <Head title="배너 관리 : 라그하임 서브 배너 - index" />

            <div className="px-6 py-4 overflow-hidden shadow-sm">
                <Link
                    href={route("kr.lh.banner.sub.create")}
