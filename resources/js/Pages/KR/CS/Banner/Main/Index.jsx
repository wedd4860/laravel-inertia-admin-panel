...
const Index = ({ auth, banners, queryParams = null, success }) => {
    ...
    const onSubmit = (e) => {
        ...
        destroy(route("kr.cs.banner.main.destroy", data.idx), {
            preserveScroll: true,
            onSuccess: ({ props }) => {
                if (props.success != "") {
    ...
    return (
        <AuthenticatedLayout member={auth.member}>
            <Head title="배너/슬라이드 관리 : 고객센터 메인슬라이드 - index" />

            <div className="px-6 py-4 overflow-hidden shadow-sm">
                {banners.data.length < 6 && (
                    <Link
                        href={route("kr.cs.banner.main.create")}
